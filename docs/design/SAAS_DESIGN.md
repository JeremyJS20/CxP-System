...
**The "Micro-Border" Rule:** 1px solid, low-opacity borders are strictly required to delineate structural layout sectioning. Boundaries must be crisp and clearly defined to support high-density data. When a border is legally/accessibly required, use a "Ghost Border" (10% opacity).

**High-Density Grids:** Use tight, symmetrical margins (4px to 8px between elements) to maximize information density and create a developer-focused, CLI-like feel.
...
|---|---|---|
| Glass blur | 16px `backdrop-filter` | 12px `backdrop-filter` |
| Elevation | Luminous border glow | Soft `box-shadow` drop shadows |
...
| Background gradient | Radial cyan aurora top, teal bottom | Subtle radial `#E0F2FE` top, `#F0F4F8` base |
...
|---|---|---|---|
| 1 | `#00B4D8` | `#0E8E9A` | Primary (brand cyan/teal) |
| 2 | `#48CAE4` | `#0D6B7A` | Secondary (lighter/deeper) |
